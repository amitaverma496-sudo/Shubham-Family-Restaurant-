    ...
    try {
      await fetch('/api/guests', {
        ...
        body: JSON.stringify({
          ...
          photoURL: firebaseUser.photoURL || `https://api.dicebear.com/7.x/bottts/svg?seed=${encodeURIComponent(firebaseUser.email)}`,
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        ...
        // Securely post profile details to Postgres SQL database
        await syncUserToSql(firebaseUser);
      } else {
        setUser(null);
        setAuthLoading(false);
  ...
  const handleSignInGoogle = async () => {
    ...
    try {
      ...
      try {
        ...
        setGateError("Google login attempt failed. Please allow popups or try again.");
      }
    } finally {
            ...
            >
              ...
              <div className="space-y-6">
                ...
                <p className="text-xs text-white/60 tracking-wider leading-relaxed font-sans max-w-sm mx-auto">
                  To protect reservation databases, confirm table bookings, and grant executive suite clearance, please verify your identity via secure Google Sign-In.
                </p>
                ...
                <div className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full h-12 rounded-full bg-white text-black font-semibold text-xs tracking-wider uppercase transition-all duration-300 hover:bg-gold hover:text-black flex items-center justify-center gap-3 shadow-[0_4px_20px_rgba(255,255,255,0.05)] hover:shadow-[0_4px_25px_rgba(212,175,55,0.25)] active:scale-[0.98] disabled:opacity-50 cursor-pointer"
                  >
                    ...
                    {gateSubmitting ? 'Verifying Identity...' : 'Sign In with Google'}
                  </button>
                </div>
