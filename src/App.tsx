...
export default function App() {
  ...
  const [user, setUser] = useState<any>(() => {
    try {
      const stored = localStorage.getItem('royal_guest_user');
  ...
  // Form input states for the Sign-In Gate
  const [gateEmail, setGateEmail] = useState('');
  const [gatePhone, setGatePhone] = useState('');
  const [gateError, setGateError] = useState('');
  const [gateSubmitting, setGateSubmitting] = useState(false);

  // Global Capturing Event Listener Intercepting "ANY BUTTON" interactions
  useEffect(() => {
    const handleGlobalClick = (e: MouseEvent) => {
      // If user is already authenticated, allow all interactions normally
      if (user) return;
      ...
      while (target && target !== document.body) {
        ...
        // Prevent blocking clicks inside the input fields or the Sign-In gate itself
        if (
          target.closest('.signin-gate-input') || 
    ...
    document.addEventListener('click', handleGlobalClick, true); // Capture phase externals
    return () => {
  ...
  const handleCustomSignIn = async (e: React.FormEvent) => {
    ...
    if (!gateEmail || !gatePhone) {
      setGateError('Please fill in both email and phone details.');
      return;
    }

    if (!gateEmail.toLowerCase().includes('@gmail.com')) {
      setGateError('Verification requires an authentic @gmail.com address.');
      return;
    }

    const cleanedPhone = gatePhone.replace(/\D/g, '');
    if (cleanedPhone.length < 8) {
      setGateError('Please enter a valid phone number (minimum 8 digits).');
      return;
    }
    ...
    try {
      const res = await fetch('/api/guests', {
        ...
        body: JSON.stringify({
          email: gateEmail.toLowerCase().trim(),
      ...
      if (res.ok) {
        const guestData = {
          email: gateEmail.toLowerCase().trim(),
          phone: gatePhone.trim(),
          displayName: gateEmail.split('@')[0],
          photoURL: `https://api.dicebear.com/7.x/bottts/svg?seed=${encodeURIComponent(gateEmail)}`
        };
        localStorage.setItem('royal_guest_user', JSON.stringify(guestData));
        setUser(guestData);
        ...
        setGateError(errObj.error || 'Identity registration failed.');
      }
    } catch (err) {
      console.error('Sign-in error:', err);
      setGateError('Network connection failure to the Cloud SQL database.');
    } finally {
  ...
  const handleSignOutGoogle = async () => {
    try {
      localStorage.removeItem('royal_guest_user');
      setUser(null);
  ...
  return (
    <div ref={containerRef} onMouseMove={handleMouseMove} className="selection:bg-gold selection:text-black min-h-screen bg-lux-black relative">
      ...
      {/* SECURE SQL GUEST IDENTITY VERIFICATION GATE OVERLAY */}
      <AnimatePresence>
        {showSignInGate && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            <motion.div
              ...
              className="w-full max-w-md bg-[#0D0D0D] border border-gold/20 rounded-3xl p-8 text-center relative shadow-[0_0_50px_rgba(212,175,55,0.15)] overflow-hidden"
            >
              {/* Background elegant golden radial glow */}
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-64 h-64 bg-gold/5 rounded-full blur-[100px] pointer-events-none" />

              {/* Close Button to return back to viewing mode safely */}
              <button
                id="google-signout-btn" 
                onClick={() => setShowSignInGate(false)}
                className="absolute top-5 right-5 text-white/45 hover:text-white transition-colors p-1"
              ...
              <div className="space-y-6">
                {/* Monogrammed verification seal */}
                <div className="w-16 h-16 rounded-full border border-gold/40 flex items-center justify-center mx-auto bg-black shadow-[0_0_30px_rgba(212,175,55,0.1)]">
                  <span className="font-serif font-black text-gold text-2xl tracking-widest pl-1">S</span>
                </div>

                <div className="space-y-2">
                  <h2 className="font-serif text-white tracking-[0.2em] text-sm uppercase">
                    SHUBHAM LUXURY
                  </h2>
                  <p className="text-[10px] tracking-[0.4em] text-gold uppercase font-mono">
                    GUEST IDENTITY REGISTRATION
                  </p>
                </div>

                <p className="text-xs text-white/60 tracking-wider leading-relaxed font-sans max-w-sm mx-auto">
                  To protect reservation databases, confirm table bookings, and grant executive suite clearance, please register your Gmail & Contact Phone to continue.
                </p>

                {/* Tactical Custom Login Form */}
                <form onSubmit={handleCustomSignIn} className="space-y-4 text-left">
                  {gateError && (
                    <div className="p-3 bg-red-950/40 border border-red-500/20 rounded-xl text-center">
                      <p className="text-xs text-red-400 font-sans">{gateError}</p>
                    </div>
                  )}

                  <div className="space-y-1">
                    <label className="block text-[10px] text-white/45 uppercase tracking-widest font-mono">
                      Gmail Address
                    </label>
                    <input
                      type="email"
                      ...
                      placeholder="e.g. master@gmail.com"
                      className="signin-gate-input w-full h-11 px-4 rounded-xl bg-black border border-white/10 text-white text-xs placeholder:text-white/20 focus:outline-none focus:border-gold/40 transition-colors"
                      required
                    />
                  </div>
                  ...
                  <button
                    ...
                    className="w-full h-11 rounded-xl bg-gold text-black font-semibold text-xs tracking-wider uppercase transition-all duration-300 hover:bg-white hover:text-black flex items-center justify-center gap-2 shadow-[0_4px_20px_rgba(212,175,55,0.15)] active:scale-[0.98] disabled:opacity-50 cursor-pointer mt-2"
                  >
                    {gateSubmitting ? 'Verifying Credentials...' : 'Register Secure Connection'}
                  </button>
                </form>

                <p className="text-[9px] text-white/30 tracking-widest font-mono uppercase">
                  Secured via Google Cloud SQL Relational Storage
                </p>
              </div>
            </motion.div>
          </motion.div>
              ...
              <div className="max-w-6xl mx-auto px-4 z-20 text-center space-y-8 select-none">
                ...
                >
                  <button
                    ...
                    className="w-full sm:w-auto px-8 py-3.5 rounded-full bg-gold hover:bg-gold/90 text-black text-[11px] font-extrabold tracking-[0.2em] uppercase transition-all duration-300 shadow-[0_4px_20px_rgba(212,175,55,0.3)] hover:shadow-[0_4px_30px_rgba(212,175,55,0.5)] cursor-pointer"
                  >
                    Book Royal Table
                  </button>
                  <button
                    onClick={() => scrollToSection('menu')}
                    className="w-full sm:w-auto px-8 py-3.5 rounded-full border border-white/20 hover:border-gold hover:bg-gold hover:text-black text-[11px] text-white font-extrabold tracking-[0.2em] uppercase transition-all duration-300 bg-black/40 backdrop-blur-sm cursor-pointer"
                  >
                    View Gourmet Menu
                  </button>
                </motion.div>
